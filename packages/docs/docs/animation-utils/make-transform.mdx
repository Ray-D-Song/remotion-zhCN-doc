...
应用一系列变换函数以生成组合的 CSS `transform` 属性。

## API

接受一个字符串数组（从下面的变换函数生成）并将它们组合成一个字符串。
...
- [此函数的源代码](https://github.com/remotion-dev/remotion/blob/main/packages/animation-utils/src/transformation-helpers/make-transform/index.ts)
