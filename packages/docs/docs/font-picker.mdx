...
id: font-picker
title: 构建 Google 字体选择器
crumb: "构建视频应用"
---

要在字体选择器中显示各种 Google 字体并在用户选择后加载它们，首先安装 `@remotion/google-fonts`（至少 v3.3.64）。

此功能仅在将 `@remotion/google-fonts` 导入为 ES 模块时才可用。  
如果将其导入为 CommonJS 模块，加载字体将会抛出错误。

调用 [`getAvailableFonts()`](/docs/google-fonts/get-available-fonts) 来获取 Google 字体列表，并调用 `.load()` 来加载字体的元数据。  
之后，在检索到的对象上：

- 您可以调用 `getInfo()` 来检索可用的样式和权重。
- 您可以调用 `loadFont()` 来加载字体本身。

使用 `fontFamily` CSS 属性来应用字体。

请记住，如果您想使用该字体渲染视频，还需要在 Remotion 视频中加载该字体。您可以通过相同的方式来完成，遍历可用字体，找到要加载的字体，然后加载它。

## 在字体选择器中显示所有字体

以下代码片段呈现一个下拉菜单，其中包含所有 Google 字体，并在选择后加载其中一个。列表中大约有 1400 种字体。
...
## 仅显示前 250 种最受欢迎的 Google 字体

为了减小捆绑包大小，您可以限制选择。不要调用 `getAvailableFonts()`，而是创建一个包含以下内容的文件，并将其用作字体数组：
