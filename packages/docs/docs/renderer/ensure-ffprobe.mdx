...
检查 `ffprobe` 二进制文件是否已安装，如果没有安装，[将其下载并放入您的 `node_modules` 文件夹](/docs/ffmpeg)。
...
您可能不需要调用此函数。如果尝试渲染且未找到二进制文件，则 Remotion 将自动下载 `ffprobe`。

如果需要在第一次渲染开始之前准备好 `ffprobe`，则此函数很有用。

还要调用 [`ensureFfmpeg()`](/docs/renderer/ensure-ffmpeg) 来获取 Remotion 需要的两个二进制文件。
...
- CLI 等效命令：[`npx remotion install ffprobe`](/docs/cli/install)
